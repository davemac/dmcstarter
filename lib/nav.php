...
// this walker applies Off-Canvas Multilevel Menu styles to the Off-Canvas menu
// http://foundation.zurb.com/sites/docs/v/5.5.3/components/offcanvas.html#off-canvas-multilevel-menu
class dmcstarter_offcanvas_walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( $depth > 0 ) {
			...
			$output .= $indent . ( $this->nav_bar['has_slide_outs'] == true ? '<li class="divider"></li>' : '' ) . '<' . $this->nav_bar['item_type'] . ' id="menu-item-' . $item->ID . '"' . $value . $class_names . '>';
		}
